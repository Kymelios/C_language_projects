...
struct Node *deleteNode(struct Node *head, int targetValue, bool isAtTheBeginning) // Deletes a node at the specified location
{
    ...
    /* 
    - if after first not NULL node is a second not NULL node,
    we save it temp and assign to the first next node in order to delete a first node
    (that is in front of the target number)

    - if second node is NULL that means the end of list,
    then we just assign deleteNode to the first node
    */
...
int main()
{
    ...
    head = insertNewNodeAtTheBeginning(head, 14);

    insertNewNodeAtTheEnd(head, 800);
